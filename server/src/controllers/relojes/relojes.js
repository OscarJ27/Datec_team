...
let controller = {
    ...
    list: function (req, res) {
        ...
        db.collection("Reloj").find().toArray(
            (error, dataProductos) => {
    ...
    find: function (req, res) {
        ...
        db.collection("Reloj").find({productoId: parseInt(req.params.id)}).toArray(
            (error, dataProductos) => {
                if (error || !dataProductos) {
                    return res.status(404).send({
    ...
    save: function (req, res) {
        ...
        if (req.body.productoId == "0") {
            db.collection("Reloj").estimatedDocumentCount().then(
                countProductos => {
                    let producto = {}
            ...
            db.collection("Reloj").updateOne({productoId: {$eq: parseInt(req.body.productoId)}},
                                                        {$set: producto},
                (error, result) =>{
                    if (error){
                        return res.status(404).send({
